...
export default function FormEnviarRecurso() {
  ...
  const onSubmit = (data) => {
    ...
    if (materiaLleno) {
      ...
      alertify.alert(
        config.nombre,
        "Debe seleccionar al menos un año y seleccionar la asignatura correspondiente."
      );
    }
  };
  ...
  const handleValidarEducatico = (e) => {
    ...
    if (res !== true) {
      alertify.alert(
        "La url del recruso debe provenir de educatico.",
        function () {
          console.log("Aceptar");
        }
      );
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          {errors.id_nivel && <p className="error">* Nivel requerido</p>}
          {
            ...
            (idNivel === 2 || idNivel === 3 || idNivel === 6) && (
              <div className="input-group mb-3">
                <div className="input-group-prepend">
                  <label className="input-group-text" htmlFor="selMateria">
                    Asignatura
                  </label>
                </div>
                <select
                  ...
                  </option>
                  {
                    ...
                    idNivel === 2 &&
                      asignaturaPrimaria.map((item, i) => (
                        <option key={"asignatura" + i} value={item.nombre}>
                          {" "}
                          {item.nombre}{" "}
                        </option>
                      ))
              ...
              <div className="input-group mb-3">
                ...
                >
                  ...
                  {programasAe.map((item, i) => (
                    <option key={"programaae" + i} value={item.idPrograma}>
                      {" "}
                      {item.nombrePrograma}{" "}
                    </option>
                  ))}
...

          {
            ...
            idNivel === 7 && idPrograma === "1" && (
              <div className="input-group mb-3">
                <div className="input-group-prepend">
                  <label className="input-group-text" htmlFor="selSubprograma">
                    Subprograma
                  </label>
                </div>
                <select
                ...
                >
                  <option defaultValue value={-1}>
                    Seleccione un subprograma
                  </option>
                  {subprogramasAe.map((item, i) => (
                    <option key={"subprograma" + i} value={item.idSubprograma}>
                      {" "}
          ...
          <div className="input-group mb-3">
            ...
            <input
              ...
              id="txtNombre"
              aria-label="Default"
              placeholder="Escriba aquí el nombre del recurso."
              ref={register({
                required: true,
                max: 128,
                min: 0,
          ...
          {errors.img_educatico && (
            <p className="error">* URL imagen requerido</p>
          )}
          ...
          {errors.descripcion && (
            <p className="error">* Descripción requerida</p>
          )}
          <br />
          ...
          <div className="pretty p-switch p-fill">
            <input type="checkbox" id="chkApoyo" name="apoyo" ref={register} />
            <div className="state">
